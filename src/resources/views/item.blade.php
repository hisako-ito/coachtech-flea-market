                        ...
                        <div class="information__content">
                            ...
                            <span class="category__list">
                                @foreach($categories as $category)
                                    <label><input type="checkbox" name="categories[]" value="{{ $category->id }}" {{ $item->categories->contains('id', $category->id) ? 'checked' : '' }}> {{ $category->name }}         {{-- デバッグ用 --}}
        ({{ $item->categories->contains('id', $category->id) ? 'checked' : 'not checked' }})</label>
                                @endforeach
                            </span>
                            <h5 class="information__condition">商品の状態</h5>
                            <span class="condition__content">{{ $item['condition']['content'] }}</span>
                        </div>
                    </div>
